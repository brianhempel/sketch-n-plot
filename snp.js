...
function select(snp_state, key) {
    ...
    Array.from(snp_state.sidebar.children).forEach(el => el.open = false);
...
function selected_sidebar_item(snp_state, key) {
    if (key === undefined) {
        return undefined;
    }
    else if ("name" in key) {
        return Array.from(snp_state.sidebar.children).find(el => el.artistName === key.name);
    }
    else if ("func_code" in key) {
        const target = [key.func_code, key.call_num];
        return Array.from(snp_state.sidebar.children).find(el => Array.from(el.children).some(child => child.funcCodeAndNum && equal_by_json(child.funcCodeAndNum, target)));
    }
}
...
function arg_to_widget(sync_editor_and_output, code, arg_type, code_type, type_compatible_local_names) {
...
function arg_to_widgets(sync_editor_and_output, code, arg_type, code_type) {
...
function catalog_open_artists(snp_state) {
    open_artists_after_execute = Array.from(snp_state.sidebar.children).filter(el => el.open).map(el => el.artistName);
}
...
function loced_widget_from_code(call_info, cell_lineno, cm, snp_state) {
    ...
    return {
        ...
        end_pos: end_pos,
        mark: mark,
        method_name: (callee.name || "").split(" ")[0] || "",
        call_info: call_info,
        widget: widget
    };
}
...
function attach_snp(snp_outer, cell_lineno, provenance_is_off_by_n_lines, user_call_info, sidebar_stuff) {
    ...
    const possible_targets = Array.from(snp_state.hover_regions_svg().querySelectorAll('[data-artist-names]')).filter(el => JSON.parse(el.dataset.artistNames).length > 0);
    ...
    if ("open_artists_after_execute" in window && open_artists_after_execute !== undefined) {
        Array.from(snp_state.sidebar.children).filter(el => open_artists_after_execute.includes(el.artistName)).forEach(el => el.open = true);
...
function build_sidebar(snp_state) {
    ...
    selectable_artists.forEach(({ id, names }) => {
        ...
        const call_widgets = artist_calls.map(call_info => {
            ...
            widget.style.display = "inline-block";
            if (is_single_call) {
                return make_el("span", { funcCodeAndNum: func_code_and_num }, { display: "grid", gridTemplateColumns: "min-content 333px" }, {}, [
    ...
    snp_state.hover_regions_svg().querySelectorAll('[data-artist-names],[data-func-code-and-num]').forEach(hover_region => {
        ...
        if (sidebar_item !== undefined) {
            ...
            hover_region.addEventListener("mouseleave", ev => {
                if (snp_state.canvas_selection !== undefined && equal_by_json(shape_selection_key(hover_region), snp_state.canvas_selection)) {
                    return;
                }
